...
class CarbonKubeStack(Stack):
    ...
    def _create_node_groups(self):
        ...
        # High-performance GPU node group for intensive ML workloads (A100 GPUs)
        gpu_high_perf_node_group = self.cluster.add_nodegroup_capacity(
            "GPUHighPerfNodes",
            instance_types=[ec2.InstanceType("p4d.24xlarge")],  # 8x A100 GPUs, 96 vCPUs, 1152 GB RAM
            min_size=0,
            max_size=3,
            desired_size=0,  # Scale from zero - expensive instances
            subnets=ec2.SubnetSelection(subnet_type=ec2.SubnetType.PRIVATE_WITH_EGRESS),
            labels={
                "node-type": "gpu-high-perf",
                "carbon-kube/workload-type": "gpu-intensive",
                "carbon-kube/migratable": "true",
                "carbon-kube/gpu-type": "A100",
                "carbon-kube/gpu-count": "8",
        ...
        # Medium GPU node group for standard ML workloads (V100 GPUs)
        gpu_medium_node_group = self.cluster.add_nodegroup_capacity(
            "GPUMediumNodes", 
            instance_types=[ec2.InstanceType("p3.2xlarge")],  # 1x V100 GPU, 8 vCPUs, 61 GB RAM
            min_size=0,
            max_size=5,
            desired_size=0,  # Scale from zero
            subnets=ec2.SubnetSelection(subnet_type=ec2.SubnetType.PRIVATE_WITH_EGRESS),
            labels={
                "node-type": "gpu-medium",
                "carbon-kube/workload-type": "gpu-standard",
                "carbon-kube/migratable": "true",
                "carbon-kube/gpu-type": "V100",
                "carbon-kube/gpu-count": "1",
            ...
            # Enable GPU support
            ami_type=eks.NodegroupAmiType.AL2_X86_64_GPU,
            capacity_type=eks.CapacityType.SPOT,  # Use spot for cost efficiency
        )
        
        # Inference GPU node group for serving workloads (T4 GPUs)
        gpu_inference_node_group = self.cluster.add_nodegroup_capacity(
            "GPUInferenceNodes",
            instance_types=[ec2.InstanceType("g4dn.2xlarge")],  # 1x T4 GPU, 8 vCPUs, 32 GB RAM
            min_size=0,
            max_size=8,
            desired_size=0,  # Scale from zero
            subnets=ec2.SubnetSelection(subnet_type=ec2.SubnetType.PRIVATE_WITH_EGRESS),
            labels={
                ...
                "carbon-kube/workload-type": "gpu-inference",
                "carbon-kube/migratable": "true",
                "carbon-kube/gpu-type": "T4",
                "carbon-kube/gpu-count": "1",
