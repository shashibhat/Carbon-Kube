...
.PHONY: gpu-eval
gpu-eval: setup-python ## Run GPU evaluation with baselines and ablations
	@echo "Running GPU evaluation harness..."
	@echo "Deploying GPU workloads and running baselines vs ablations..."
	
	# Deploy GPU workloads
	kubectl apply -f test/workloads/gpu/bert-finetune.yaml
	kubectl apply -f test/workloads/gpu/llama-inference.yaml
	kubectl apply -f test/workloads/gpu/rapids-tpcds.yaml
	kubectl apply -f test/workloads/gpu/flink-nexmark.yaml
	
	# Run baseline evaluations (HPA/static scheduling)
	@echo "Running HPA baseline..."
	$(VENV_DIR)/bin/python evaluation/run_baseline.py --scheduler=hpa --workloads=gpu --duration=30m
	
	@echo "Running static baseline..."
	$(VENV_DIR)/bin/python evaluation/run_baseline.py --scheduler=static --workloads=gpu --duration=30m
	
	# Run ablation studies
	@echo "Running no-RL ablation..."
	$(VENV_DIR)/bin/python evaluation/run_ablation.py --disable=rl --workloads=gpu --duration=30m
	
	@echo "Running no-MIG ablation..."
	$(VENV_DIR)/bin/python evaluation/run_ablation.py --disable=mig --workloads=gpu --duration=30m
	
	# Run full Carbon-Kube evaluation
	@echo "Running Carbon-Kube GPU evaluation..."
	$(VENV_DIR)/bin/python evaluation/run_evaluation.py --workloads=gpu --duration=30m --seeds=10
	
	# Generate comparison report
	@echo "Generating evaluation report..."
	$(VENV_DIR)/bin/python evaluation/generate_report.py --output=results/gpu-eval-report.html
	
	@echo "GPU evaluation complete! Results available in results/gpu-eval-report.html"
